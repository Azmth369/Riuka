...
// The whole chat pipeline now runs server-side (POST /ask); this dropdown
// just picks which provider the server tries first, the other is fallback.
...
// When the server is configured with RUKA_ACCESS_TOKEN, every API route
// needs the X-Access-Token header. The token is asked for once (only after
// the server actually rejects a request), remembered in localStorage, and
// only re-asked for if the remembered value stops being accepted.
let accessToken = '';
try{ accessToken = localStorage.getItem('warroom_access_token') || ''; }catch(e){}
let lastRejectedToken = null;
let tokenPromptOpen = false;

function promptForAccessToken(){
  if(tokenPromptOpen) return null;
  tokenPromptOpen = true;
  const entered = window.prompt('This server requires an access token. Enter it once — it will be remembered in this browser.');
  tokenPromptOpen = false;
  if(entered == null || !entered.trim()) return null;
  accessToken = entered.trim();
  try{ localStorage.setItem('warroom_access_token', accessToken); }catch(e){}
  return accessToken;
}

// Every API call in this app goes through here so the header (when needed)
// is attached everywhere, once — and a 401 offers a chance to enter the
...
async function apiFetch(url, options){
  ...
  if(res.status === 401){
    const fresh = promptForAccessToken();
    if(fresh && fresh !== lastRejectedToken){
      opts.headers['X-Access-Token'] = fresh;
      res = await fetch(url, opts);
      if(res.status === 401) lastRejectedToken = fresh;
    }
  }
  return res;
}
...
async function cocFetch(path){
  let res;
  try{
    res = await apiFetch(`${LOCAL_PROXY}/coc${path}`);
  }catch(e){
...
async function loadCwlLeague(tagPath){
  ...
  for(let i = 0; i < group.rounds.length; i++){
    ...
    // Keep WHICH war tag is ours, not just the war object — the attack-log
    // sync needs it (the server skips CWL attack rows sent without a
    // warTag) and cwl_rounds.war_tag stores it for the season history.
    const ourEntry = wars.map((w, i) => ({ w, tag: tags[i] }))
      .find(e => e.w && (e.w.clan?.tag === ourTag || e.w.opponent?.tag === ourTag));
    if(!ourEntry) continue; // our war for this round hasn't been fetched/paired yet
    const ourWar = ourEntry.w;
