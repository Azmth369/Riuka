...
// The dropdown is now just a preference: it is sent to POST /ask, and the
// server tries that provider first, falling back to the other one
// automatically on failure (see lib/chatEngine.js).
...
// --- Access token (shared secret) ------------------------------------------
// When RUKA_ACCESS_TOKEN is set on the server, every API route requires it
// in an X-Access-Token header. The page itself always loads; the first API
// call that comes back 401 asks for the token once, stores it locally, and
// the request is retried. All API calls below go through apiFetch().
const TOKEN_KEY = 'warroom_access_token';
...
let _tokenPromptBusy = false;
async function askForAccessToken(){
  if(_tokenPromptBusy) return null;
  _tokenPromptBusy = true;
  try{
    const entered = window.prompt('This server is locked. Enter the access token:');
    if(entered && entered.trim()){
      localStorage.setItem(TOKEN_KEY, entered.trim());
      return entered.trim();
    }
  } finally { _tokenPromptBusy = false; }
  return null;
}
...
async function apiFetch(url, options = {}){
  ...
  if(res.status === 401 && !opts._authRetried){
    const token = await askForAccessToken();
    if(token){
      opts._authRetried = true;
