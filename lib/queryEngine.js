...
//
// This is the layer that makes counts/filters/rankings hallucination-proof:
// the structured result computed here is declared authoritative for the AI.
...
function stripNegatedDatasets(text = '') {
...
function buildFilters(q) {
  ...
  // "one attack left/remaining" must be captured BEFORE the "used" patterns
  // so "one attack left" is not misread as "used one attack".
  ...
  const asksLowest = /\b(?:lowest|least|minimum|min|fewest)\b/.test(q);
  ...
  return {
    ...
    metric: donation ? 'donations' : trophies ? 'trophies' : null,
    role: role ? String(role).replace(/-/g, '').replace(/^coleader$/, 'coLeader') : null,
    asks_count: count,
  };
}
...
function applyAttackUsageFilters(rows, plan) {
  let filtered = [...(rows ?? [])];
...
function deterministicWarMembers(question, members = []) {
...
function deterministicMemberMetric(question, players = []) {
  const plan = buildQueryPlan(question);
...
function deterministicRole(question, players = []) {
  const plan = buildQueryPlan(question);
...
// Pure intent executor: produces the authoritative structured result embedded
// in the AI context. The database-backed variant lives in queryRouter.
function executeIntent(question, { players = [], currentWarMembers = [] } = {}) {
  const plan = buildQueryPlan(question);
...
module.exports = {
  normalize, stripNegatedDatasets, buildQueryPlan, understandQuestion,
  applyAttackUsageFilters, deterministicWarMembers, deterministicMemberMetric,
  deterministicRole, executeIntent,
};
