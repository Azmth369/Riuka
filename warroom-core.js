...
// --- Shared-secret gate ----------------------------------------------------
// Every proxy call goes through this wrapper. When the server has
// RUKA_ACCESS_TOKEN set, every API route requires the matching
// X-Access-Token header; the token is asked for once (on the first 401),
// stored in localStorage, and attached to every request from then on.
async function proxyFetch(url, opts = {}){
  const headers = Object.assign({}, opts.headers || {});
  const tok = localStorage.getItem('warroom_access_token');
...
async function cocFetch(path){
  let res;
  try{
    res = await proxyFetch(`${LOCAL_PROXY}/coc${path}`);
  }catch(e){
